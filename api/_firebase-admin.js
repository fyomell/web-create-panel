...
if (!admin.apps.length) {
  try {
    // Cek apakah variabel ada di Vercel
    if (!process.env.FIREBASE_SERVICE_ACCOUNT_KEY) {
        throw new Error("Kunci FIREBASE_SERVICE_ACCOUNT_KEY tidak ditemukan di Vercel Environment Variables.");
    }
    // Baca dan parse kunci dari variabel Vercel
    const serviceAccount = JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT_KEY);
    
    admin.initializeApp({
      credential: admin.credential.cert(serviceAccount)
    });
    console.log("Firebase Admin SDK berhasil diinisialisasi dari Environment Variable.");
  } catch (error) {
